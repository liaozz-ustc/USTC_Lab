...
mov ecx,0
FSTCW temp
mov ax,temp ;将四舍五入改为截�?
push ax
or ax,0C00H;将RC舍入类型修改为截�?都是保留整数
mov temp,ax
FLDCW temp
FLD result;实数入栈,书上少了这句�?
FTST;�?比较
FSTSW ax
...
mov eax,whole
mov ebx,10
push ebx;作为整数是否出栈完毕的标�?
.REPEAT
    mov edx,0
    div ebx;edx保存除的余数，整数部分一直除10，余数入栈，直到�?
    add dl,'0' ;转化为ASCII�?
    push edx
.UNTIL eax==0
;栈中保存了整数部分从高位到低位的ASCII�?
...
.REPEAT
    FIMUL ten;//小数�?0
    FIST temp;保存�?0后的整数，即小数高位
    mov ax,temp
    add al,'0'
    mov data[ecx],al;存入数组
    inc ecx
    FISUB temp;小数�?0后减去整数部分，处理剩余小数
    FXAM;检测栈顶是否为0
    SAHF;传给标志寄存�?
.UNTIL ZERO?
FCOMP
