...
module HarzardUnit(
    ...
    always @(*) begin
        if (rst) begin
            ...
            bubbleF = wb_select | miss;                                                   
            bubbleD = wb_select | miss;    
            bubbleE = miss;
            bubbleM = miss;
            bubbleW = miss;
        end
    ...
    always @(*) begin
        if (rst) begin
            ...
            if (src_reg_en[1] == 1 && reg1_srcE != 5'b0 && reg1_srcE == reg_dstM && reg_write_en_MEM)   
                op1_sel = 2'b00;
            else if (src_reg_en[1] == 1 && reg1_srcE != 5'b0 && reg1_srcE == reg_dstW && reg_write_en_WB) 
                op1_sel = 2'b01;
            else if (src_reg_en[1] == 0 && alu_src1 == 1) 
                op1_sel = 2'b10;
            else
                op1_sel = 2'b11;
        
            if (src_reg_en[0] == 1 && reg2_srcE != 5'b0 && reg2_srcE == reg_dstM && reg_write_en_MEM)    
                op2_sel = 2'b00;
            else if (src_reg_en[0] == 1 && reg2_srcE != 5'b0 && reg2_srcE == reg_dstW && reg_write_en_WB) 
                op2_sel = 2'b01;
            else if (src_reg_en[0] == 0 && alu_src2 == 2'b01) 
                op2_sel = 2'b10;
            else
                op2_sel = 2'b11;

            if (reg2_srcE != 5'b0 && reg2_srcE == reg_dstM && reg_write_en_MEM)   
                reg2_sel = 2'b00;
            else if (reg2_srcE != 5'b0 && reg2_srcE == reg_dstW && reg_write_en_WB) 
                reg2_sel = 2'b01;
            else
                reg2_sel = 2'b10; 
        end
